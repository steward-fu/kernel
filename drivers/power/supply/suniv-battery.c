...
static int suniv_battery_probe(struct platform_device *pdev)
{
  ...
  lradc = (uint8_t*)ioremap(SUNIV_LRADC_BASE, 1024);
  suniv_setbits(lradc, 1);

	di->dev = &pdev->dev;
	di->bat_desc.name = "suniv-battery";
...
static const struct of_device_id miyoo_battery_of_match[] = {{.compatible = "allwinner,suniv-f1c100s-battery", },{},};
...
MODULE_DESCRIPTION("battery driver for allwinner suniv handheld");
MODULE_LICENSE("GPL");
