...
static int do_gpio_request(int pin)
{
  if(pin < 0){
    printk("%s, invalid gpio pin\n", __func__);
    return -1;
  }
  if(gpio_request(pin, "vmotor") < 0){
...
static long myioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
  ...
  case VMOTOR_ENABLE:
    ...
    break;
  case VMOTOR_SET:
    if((pin > 0) && (vmotor_enable > 0)){
      gpio_set_value(pin, arg ? 0 : 1);
    }
    break;
  }
  return 0;
...
MODULE_DESCRIPTION("pwm driver for allwinner suniv chip");
MODULE_LICENSE("GPL");
