...
#define POCKETGO_USE_EARPHONE 0
...
{
    uint32_t ret = readl(iomm.codec + DAC_MIXER_CTRL_REG) & 0x3f;
    return sprintf(buf, "%d\n", ret);
}

static ssize_t volume_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int rc = -1;
    uint32_t ret = 0;
    unsigned long vol = 0;

    rc = kstrtoul(buf, 0, &vol);
    if(rc) {
        return rc;
    }
...
static void suniv_ioremap(void)
{
    ...
    iomm.codec = (uint8_t *)ioremap(SUNIV_CODEC_BASE, 1024);
}
...
static void suniv_codec_init(void)
{
    ...
    writel((1 << 31) | (1 << 30) | (1 << 27) | (1 << 26) | (1 << 17) | (1 << 15) | (1 << 9) | (10 << 0) | 0x3f, iomm.codec + DAC_MIXER_CTRL_REG);
}
...
static void suniv_gpio_init(void)
{
    ...
    if(suniv_variant == 0) {
        ...
        ret |= 0x00000010;
        writel(ret, iomm.gpio + PA_CFG0);
        suniv_setbits(iomm.gpio + PA_DATA, (1 << 1));
    }
    else if(suniv_variant == 3) {			// FC3000_OLD
        gpio_direction_output((32 * 0) + 1, 1);	// Output
        gpio_direction_output((32 * 0) + 2, 1);	// Output
        gpio_set_value((32 * 0) + 1, 1);		// LED On
        gpio_set_value((32 * 0) + 2, 1);		// Amplifier On
    }
}
...
static struct snd_soc_dai_driver mycpu_dai = {
    ...
    .playback = {
        .stream_name = "suniv cpu playback",
        .channels_min = 2,
        .channels_max = 2,
        .rates = SNDRV_PCM_RATE_8000_192000,
        .formats = SNDRV_PCM_FMTBIT_S16_LE,
...
static int myaudio_probe(struct platform_device *pdev)
{
    ...
    if(iomm.irq < 0) {
        ...
        if(request_irq(iomm.irq, dma_irq_handler, IRQF_SHARED, "suniv_dma_irq", iomm.substream)) {
            printk("%s, failed to register DMA interrupt\n", __func__);
        }
    }
    device_create_file(&pdev->dev, &dev_attr_volume);
    return 0;
}

static const struct of_device_id myaudio_driver_of_match[] = {{.compatible = "allwinner,suniv-f1c100s-codec",}, {}};
MODULE_DEVICE_TABLE(of, myaudio_driver_of_match);

static struct platform_driver myaudio_driver = {
    .driver = {
        .name = "suniv audio driver",
...
MODULE_DESCRIPTION("audio codec driver for allwinner suniv handheld");
